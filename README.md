...
images with several browsing layouts, extended to also catalogue a film/TV/music library backed by
a local [Plex Media Server](https://www.plex.tv/) install as the metadata/artwork source.
...
Plex isn't required to use fisheye's photo-gallery features at all, and isn't meant to be a
permanent dependency even for the media side — it's used as a convenient, already-populated
metadata/artwork source (title, genre, cast, description, poster/backdrop images, external IDs) to
backfill from, one library scan at a time, rather than typing all of that in by hand.
...
- For the media extension only: a local Plex Media Server install, with its library database path
  and (for artwork/external-ID lookups specifically) an API token configured
